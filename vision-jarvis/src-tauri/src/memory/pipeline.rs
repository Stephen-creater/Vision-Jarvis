...
impl PipelineScheduler {
    ...
    pub fn start(&self) -> JoinHandle<()> {
        ...
        let backlog_retry_interval = Duration::from_secs(45); // 45秒 - 补跑历史未分析录制
        ...
        tokio::spawn(async move {
            ...
            let mut backlog_tick = interval(backlog_retry_interval);
            // 记录上次生成日总结的日期，避免重复生成
            ...
            loop {
                tokio::select! {
                    ...
                    _ = backlog_tick.tick() => {
                        if let Err(e) = Self::analyze_next_pending_recording(&db, &screenshot_analyzer).await {
                            warn!("[Pipeline] 补跑未分析录制失败: {}", e);
                        }
                    }
                }
            }
    ...
    /// Task: 追补历史未分析录制，避免 AI 或应用重启后留下永久积压
    async fn analyze_next_pending_recording(
        db: &Database,
        screenshot_analyzer: &RwLock<Option<Arc<ScreenshotAnalyzer>>>,
    ) -> Result<()> {
        let pending: Option<(String, String)> = db.with_connection(|conn| {
            Ok(conn.query_row(
                "SELECT id, path
                 FROM recordings
                 WHERE analyzed = 0
        ...
        let Some((recording_id, path)) = pending else {
            return Ok(());
        };
        ...
        info!(
            "[Pipeline] 补跑未分析录制: {} path={}",
            &recording_id[..8.min(recording_id.len())],
            path
        );
        analyzer
            .analyze_single_direct(&recording_id, std::path::Path::new(&path))
            .await?;
